...
public class SocketServer implements Runnable {
	...
	try{  
	    ...
	    ui.tfEventos.append("Servidor iniciado em >> IP : " + InetAddress.getLocalHost() + ", Porta : " + server.getLocalPort());
	    start(); 
        }
	catch(IOException ioe){  
            ui.tfEventos.append("Can not bind to port : " + port + "\nRetrying"); 
            ui.RetryStart(0);
	}
    }
	...
	while (thread != null){  
            try{  
		ui.tfEventos.append("\nAguardando conexão, JChat - Client ..."); 
	...
	if (clientCount < clients.length){  
	    ...
	    try{  
            ...
            ui.tfEventos.append("\nCliente recusado : maximum " + clients.length + " reached.");
	}
    }
}
